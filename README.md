### Hi there 👋 I'm Mostafa Abdelhakem

A **Software Engineer & Tech Educator** based in the 🇦🇪 UAE — building real-world tools,
designing AI-integrated curricula, and preparing the next generation of developers.

---

### ⚡ About Me

- 👨‍🏫 **Tech Educator @ Al Wahda Foundation** (in partnership with Athena Education)
- 🤖 Designing structured **AI Literacy curricula** aligned with CSTA standards and UAE Vision 2031
- 🛠️ Built a full **web-based classroom observation tool** for school leadership — with PDF export, auto-save & keyboard shortcuts
- 💻 Full-Stack Developer with experience in **React, Next.js, Node.js, and MongoDB**
- 📊 Exploring **Data Analysis, Power BI, and AI integration in education**
- 🧠 Strong foundation in **Data Structures, Algorithms, and System Design**
- 🚀 Former **President @ Microsoft Tech Club** (370+ members)

---

### 🧠 Interests

- 🤖 Artificial Intelligence & AI Literacy
- 🎓 Tech Education, Curriculum Design & Mentorship  
- 🌐 Full-Stack Web Development  
- 📊 Data Analytics & Visualization  
- ☁️ Cloud Computing & Scalable Systems  

---

### 💼 Experience Snapshot

- 👨‍🏫 **Tech Educator @ Al Wahda Foundation** *(UAE — Current)*
  - Teaching Python, HTML/CSS, and AI-powered development
  - Designing UAE-contextualized, project-based AI literacy lessons
  - Building internal tools to support school operations

- 👨‍🏫 **Programming Instructor @ Timedoor Academy**
  - Taught programming, AI basics, and development to students aged 5–18
  - Designed interactive, project-based learning experiences

- 🧑‍💼 **Community Manager @ E-Spaces**
  - Managed operations, client experience, and data-driven improvements

- 🏆 **President @ Microsoft Tech Club**
  - Led 370+ members across 16+ projects per season
  - Delivered workshops impacting 3,000+ students
  - Built and launched the club's website and digital platforms

---

### 🛠️ Tech Stack

#### 💻 Languages & Core
<p>
  <img src="https://skillicons.dev/icons?i=js,ts,python,java,cpp,cs" />
</p>

#### 🌐 Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,angular,tailwind,sass,bootstrap,html,css" />
</p>

#### ⚙️ Backend & Database
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,mysql" />
</p>

#### 📊 Data & AI
<p>
  <img src="https://skillicons.dev/icons?i=python" />
</p>

- Power BI · Pandas · NumPy  
- Prompt Engineering & AI Integration  
- AI Curriculum Design for K–12  

#### 🔧 Tools & Platforms
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,postman,vite,figma" />
</p>

---

### 🏆 Key Achievements

- 👨‍🏫 Teaching **200+ students** across 7 grade levels in the UAE
- 🤖 Designed and delivered a full **AI Literacy curriculum** for middle and secondary school
- 🛠️ Built a production-ready **classroom observation web app** used by school leadership
- 🚀 Led a **370-member tech organization** with 16+ active projects
- 👥 Reached **3,000+ students** through workshops and events
- 📊 Built **data-driven platforms** across education, healthcare, and business

---

### 🌍 Connect With Me

<p align="center">
  <a href="https://github.com/mostafabdelhakem">GitHub</a> •
  <a href="https://linkedin.com/in/mostafabdelhakem">LinkedIn</a> •
  <a href="https://mostafabdelhakem.github.io">Portfolio</a>
</p>

